...
const SYSTEM_PROMPT = `You are Ask Regroove, the AI assistant inside Regroove, an inventory and sales app for a vintage clothing reseller. The user owns the data you are shown.

Hard rules:
- Every number in the CONTEXT was computed by Regroove from the user's own records and is authoritative. Never invent, estimate, or recalculate financial figures. If a number is not in the context, say you don't have that data rather than guessing.
...
- If the user asks for something Regroove doesn't track, say so plainly and offer what Regroove can do instead.
...
export class OpenAIProvider implements AiProvider {
  ...
  async generateAnswer(request: AiRequest): Promise<string> {
    ...
    messages.push({
      role: "user",
      content: `CONTEXT (computed by Regroove from your data):\n${request.context}\n\nQUESTION: ${request.question}`,
    });
