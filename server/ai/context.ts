/**
 * Controlled context builders for Ask Regroove.
 *
...
async function buildOverview(
  ...
  const sections = [
    "INVENTORY (computed by Regroove)",
    `- ${total} items total: ${listed} listed, ${drafts} draft, ${sold} sold`,
    "",
    `SALES (${scopeLabel(range, "last 30 recorded")} — computed by Regroove)`,
    ...
    "",
    `EXPENSES (${scopeLabel(range, "last 20 recorded")} — computed by Regroove)`,
...
async function buildInventory(
  ...
  if (includeUnsold) {
    ...
    const sections = [
      "OLDEST UNSOLD ITEMS (computed by Regroove — days held from acquired_date)",
      oldest.length
        ? oldest
            .map(
              (i, n) =>
...
async function buildFinancials(
  ...
  return {
    context: [
      `FINANCIALS (${scopeLabel(range, "recent records")} — computed by Regroove)`,
...
async function buildExpenses(
  ...
  return {
    context: [
      `EXPENSES (${scopeLabel(range, "recent records")} — computed by Regroove)`,
...
async function buildMarketplaces(
  ...
  return {
    context: [
      `MARKETPLACE ACTIVITY (${scopeLabel(range, "recent sales")} — computed by Regroove)`,
      ...
      "",
      "CONNECTION STATUS (tracked in Regroove — not live API integrations)",
      [...connStatus.entries()]
        .map(([m, s]) => `- ${m}: ${s}`)
