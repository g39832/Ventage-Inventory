# Threadly — eBay Integration Setup
...
- It's the only marketplace in Threadly with a real API. Depop, Poshmark, Vinted, Mercari,
...
2. **Your apps → Create an application**.
   - *Application name:* anything (e.g. "Threadly Inventory").
...
> Credentials. The connect button in Threadly handles the whole flow.
...
3. Sign into Threadly → **Marketplace connections** → eBay → **Connect with eBay**.
...
When a user publishes an item, Threadly:
